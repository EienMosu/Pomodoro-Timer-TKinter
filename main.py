...
def start_timer():
    ...
    reps += 1
    if reps % 8 == 0:
        my_label_timer.config(text="Long Break", fg=PINK)
        count_down(long_break_sec)
    elif reps % 2 == 0:
        my_label_timer.config(text="Short Break", fg=PINK)
        count_down(short_break_sec)
    else:
        my_label_timer.config(text="Working Time", fg=RED)
        count_down(work_sec)
...
my_label_timer = Label(text="Timer", font=("Algerian", 50), fg=GREEN, bg=YELLOW)
my_label_timer.grid(column=1, row=0)

my_label_checkmark = Label(fg=GREEN, bg=YELLOW, font=("Arial", 20))
